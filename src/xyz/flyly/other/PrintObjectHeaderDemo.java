...
public class PrintObjectHeaderDemo {
    int state = 0;
...
//        test6();
        ...
        PrintObjectHeaderDemo demo = new PrintObjectHeaderDemo();

        long stateOffset = unsafe.objectFieldOffset(PrintObjectHeaderDemo.class.getDeclaredField("state"));

        unsafe.compareAndSwapInt(demo, stateOffset, 0, 2);
    ...
    private static void test4() throws Exception {
        ...
        new Thread(
                () -> {
                    synchronized (a) {
                        System.out.println(Thread.currentThread().getName() + "线程1对a加锁后");
                        try {
                            printLockHeader(a);
                        } catch (Exception e) {
                            e.printStackTrace();
        ...
        new Thread(
                () -> {
                    synchronized (a) {
                        System.out.println(Thread.currentThread().getName() + "线程2对a加锁后");
                        try {
                            printLockHeader(a);
                        } catch (Exception e) {
                            e.printStackTrace();
